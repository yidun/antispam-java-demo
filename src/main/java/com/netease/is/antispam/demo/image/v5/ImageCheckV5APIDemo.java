/*
 * @(#) ImageCheckAPIDemo.java 2016年3月15日
 *
 * Copyright 2010 NetEase.com, Inc. All rights reserved.
 */
package com.netease.is.antispam.demo.image.v5;

import java.util.HashMap;
import java.util.Map;
import java.util.Random;

import org.apache.http.Consts;
import org.apache.http.client.HttpClient;

import com.google.gson.JsonArray;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.netease.is.antispam.demo.utils.HttpClient4Utils;
import com.netease.is.antispam.demo.utils.SignatureUtils;
import com.netease.is.antispam.demo.utils.Utils;

/**
 * 调用易盾反垃圾云服务图片在线检测接口API示例，该示例依赖以下jar包： 1. httpclient，用于发送http请求 2. commons-codec，使用md5算法生成签名信息，详细见SignatureUtils.java
 * 3. gson，用于做json解析
 *
 * @author hzgaomin
 * @version 2016年2月3日
 */
public class ImageCheckV5APIDemo {
    /**
     * 产品密钥ID，产品标识
     */
    private final static String SECRETID = "your_secret_id";
    /**
     * 产品私有密钥，服务端生成签名信息使用，请严格保管，避免泄露
     */
    private final static String SECRETKEY = "your_secret_key";
    /**
     * 业务ID，易盾根据产品业务特点分配
     */
    private final static String BUSINESSID = "your_business_id";
    /**
     * 易盾反垃圾云服务图片在线检测接口地址
     */
    private final static String API_URL = "https://as.dun.163.com/v5/image/check";
    /**
     * 实例化HttpClient，发送http请求使用，可根据需要自行调参
     */
    private static HttpClient httpClient = HttpClient4Utils.createHttpClient(100, 20, 10000, 2000, 2000);

    /**
     * @param args
     * @throws Exception
     */
    public static void main(String[] args) throws Exception {
        Map<String, String> params = new HashMap<>();
        // 1.设置公共参数
        params.put("secretId", SECRETID);
        params.put("businessId", BUSINESSID);
        params.put("version", "v5");
        params.put("timestamp", String.valueOf(System.currentTimeMillis()));
        params.put("nonce", String.valueOf(new Random().nextInt()));
        // MD5, SM3, SHA1, SHA256
        params.put("signatureMethod", "MD5");

        // 2.设置私有参数
        JsonArray jsonArray = new JsonArray();
        // 传图片url进行检测，name结构产品自行设计，用于唯一定位该图片数据
        JsonObject image1 = new JsonObject();
        image1.addProperty("name", "https://nos.netease.com/yidun/FTX-vdv04MuDi7k8Qn5q");
        image1.addProperty("type", 1);
        // 主动回调地址url,如果设置了则走主动回调逻辑
        // image1.addProperty("callbackUrl", "http://***");
        image1.addProperty("data", "https://nos.netease.com/yidun/FTX-vdv04MuDi7k8Qn5q");
        jsonArray.add(image1);

        // 传图片base64编码进行检测，name结构产品自行设计，用于唯一定位该图片数据
        JsonObject image2 = new JsonObject();
        image2.addProperty("name", "{\"imageId\": 33451123, \"contentId\": 78978}");
        image2.addProperty("type", 2);
        // 主动回调地址url,如果设置了则走主动回调逻辑
        // image2.addProperty("callbackUrl", "http://***");
        image2.addProperty("data",
                "");
        jsonArray.add(image2);

        params.put("images", jsonArray.toString());
        // params.put("account", "java@163.com");
        // params.put("ip", "123.115.77.137");

        // 预处理参数
        params = Utils.pretreatmentParams(params);
        // 3.生成签名信息
        String signature = SignatureUtils.genSignature(SECRETKEY, params);
        params.put("signature", signature);

        // 4.发送HTTP请求，这里使用的是HttpClient工具包，产品可自行选择自己熟悉的工具包发送请求
        String response = HttpClient4Utils.sendPost(httpClient, API_URL, params, Consts.UTF_8);

        // 5.解析接口返回值
        JsonObject resultObject = new JsonParser().parse(response).getAsJsonObject();
        int code = resultObject.get("code").getAsInt();
        String msg = resultObject.get("msg").getAsString();
        if (code == 200) {
            // 图片反垃圾结果
            JsonArray resultArray = resultObject.getAsJsonArray("result");
            for (JsonElement jsonElement : resultArray) {
                JsonObject jObject = jsonElement.getAsJsonObject();
                JsonObject antispam = jObject.get("antispam").getAsJsonObject();
                String name = antispam.get("name").getAsString();
                String taskId = antispam.get("taskId").getAsString();
                int status = antispam.get("status").getAsInt();
                // 检测状态 0 未开始、1检测中、2检测成功、3检测失败
                if (2 == status) {
                    // 图片维度结果
                    int suggestion = antispam.get("suggestion").getAsInt();
                    JsonArray labelArray = antispam.get("labels").getAsJsonArray();
                    System.out.printf("taskId=%s，status=%s，name=%s，suggestion=%s%n", taskId, status, name, suggestion);
                    // 产品需根据自身需求，自行解析处理，本示例只是简单判断分类级别
                    for (JsonElement labelElement : labelArray) {
                        JsonObject lObject = labelElement.getAsJsonObject();
                        int label = lObject.get("label").getAsInt();
                        int level = lObject.get("level").getAsInt();
                        double rate = lObject.get("rate").getAsDouble();
                        JsonArray subLabels = lObject.getAsJsonArray("subLabels");
                        if (null != subLabels) {
                            System.out.printf("label:%s, level=%s, rate=%s, subLabels=%s%n", label, level,
                                    rate, subLabels.toString());
                        } else {
                            System.out.printf("label:%s, level=%s, rate=%s%n", label, level, rate);
                        }
                    }
                    switch (suggestion) {
                        case 0:
                            System.out.println("#图片机器检测结果：最高等级为\"正常\"\n");
                            break;
                        case 1:
                            System.out.println("#图片机器检测结果：最高等级为\"嫌疑\"\n");
                            break;
                        case 2:
                            System.out.println("#图片机器检测结果：最高等级为\"确定\"\n");
                            break;
                        default:
                            break;
                    }
                } else {
                    // 检测状态 0 未开始、1检测中、2检测成功、3检测失败
                    System.out.printf("图片检测失败，taskId=%s，status=%s，name=%s%n", taskId, status, name);
                }
                // 图片OCR结果
                if (null != jObject.get("ocr")) {
                    JsonObject ocr = jObject.get("ocr").getAsJsonObject();
                    JsonArray ocrDetails = ocr.get("details").getAsJsonArray();
                    // 产品需根据自身需求，自行解析处理，本示例只是简单输出ocr结果信息
                    for (JsonElement detail : ocrDetails) {
                        JsonObject lObject = detail.getAsJsonObject();
                        String content = lObject.get("content").getAsString();
                        JsonArray lineContents = lObject.getAsJsonArray("lineContents");
                        System.out.printf("识别ocr文本内容:%s, ocr片段及坐标信息:%s%n", content, lineContents.toString());
                    }
                }
                // 图片人脸检测结果
                if (null != jObject.get("face")) {
                    JsonObject face = jObject.get("face").getAsJsonObject();
                    JsonArray faceDetails = face.get("details").getAsJsonArray();
                    // 产品需根据自身需求，自行解析处理，本示例只是简单输出人脸结果信息
                    for (JsonElement detail : faceDetails) {
                        JsonObject lObject = detail.getAsJsonObject();
                        int faceNumber = lObject.get("faceNumber").getAsInt();
                        JsonArray faceContents = lObject.getAsJsonArray("faceContents");
                        System.out.printf("识别人脸数量:%s, 人物信息及坐标信息:%s%n", faceNumber, faceContents.toString());
                    }
                }
                // 图片质量检测结果
                if (null != jObject.get("quality")) {
                    JsonObject quality = jObject.get("quality").getAsJsonObject();
                    JsonArray qualityDetails = quality.get("details").getAsJsonArray();
                    // 产品需根据自身需求，自行解析处理，本示例只是简单输出质量结果信息
                    for (JsonElement detail : qualityDetails) {
                        JsonObject lObject = detail.getAsJsonObject();
                        float aestheticsRate = lObject.get("aestheticsRate").getAsFloat();
                        float sharpnessRate = lObject.get("sharpnessRate").getAsFloat();
                        JsonObject metaInfo = lObject.getAsJsonObject("metaInfo");
                        JsonObject boarderInfo = lObject.getAsJsonObject("boarderInfo");
                        JsonObject backgroundInfo = lObject.getAsJsonObject("backgroundInfo");
                        System.out.printf("图片美观度分数:%s, 图片清晰度分数:%s, 图片基本信息:%s, 图片边框信息:%s, 图片背景信息:%s%n", aestheticsRate,
                                sharpnessRate, metaInfo.toString(), boarderInfo.toString(), backgroundInfo.toString());
                    }
                }
                // 图片logo结果
                if (null != jObject.get("logo")) {
                    JsonObject logo = jObject.get("logo").getAsJsonObject();
                    JsonArray logoDetails = logo.get("details").getAsJsonArray();
                    // 产品需根据自身需求，自行解析处理，本示例只是简单输出logo结果信息
                    for (JsonElement detail : logoDetails) {
                        JsonObject lObject = detail.getAsJsonObject();
                        String logoName = lObject.get("logoName").getAsString();
                        float x1 = lObject.get("x1").getAsFloat();
                        float y1 = lObject.get("y1").getAsFloat();
                        float x2 = lObject.get("x2").getAsFloat();
                        float y2 = lObject.get("y2").getAsFloat();
                        System.out.printf("logo信息:%s, 左上角点x坐标:%s, 左上角点y坐标:%s, 右下角点x坐标:%s, 右下角点y坐标:%s%n", logoName, x1,
                                y1,
                                x2, y2);
                    }
                }
                // 图片识别结果
                if (null != jObject.get("discern")) {
                    JsonObject discern = jObject.get("discern").getAsJsonObject();
                    JsonArray discernDetails = discern.get("details").getAsJsonArray();
                    // 产品需根据自身需求，自行解析处理，本示例只是简单输出图片识别结果信息
                    for (JsonElement detail : discernDetails) {
                        JsonObject lObject = detail.getAsJsonObject();
                        int type = lObject.get("type").getAsInt();
                        String discernName = lObject.get("discernName").getAsString();
                        float rate = lObject.get("rate").getAsFloat();
                        switch (type) {
                            case 1:
                                System.out.printf("识别物体类型:场景识别, 识别名称:%s, 分数:%s%n", discernName, rate);
                                break;
                            case 2:
                                System.out.printf("识别物体类型:地标识别, 识别名称:%s, 分数:%s%n", discernName, rate);
                                break;
                            case 3:
                                System.out.printf("识别物体类型:百度识图, 识别名称:%s, 分数:%s%n", discernName, rate);
                                break;
                            default:
                                break;
                        }
                    }
                }
                // 用户画像结果
                if (null != jObject.get("userRisk")) {
                    JsonObject userRisk = jObject.get("userRisk").getAsJsonObject();
                    JsonArray userRiskDetails = userRisk.get("details").getAsJsonArray();
                    // 产品需根据自身需求，自行解析处理，本示例只是简单输出用户画像结果信息
                    for (JsonElement detail : userRiskDetails) {
                        JsonObject lObject = detail.getAsJsonObject();
                        String account = lObject.get("account").getAsString();
                        int accountLevel = lObject.get("accountLevel").getAsInt();
                        switch (accountLevel) {
                            case 0:
                                System.out.printf("账号:%s, 用户账号风险级别:正常%n", account);
                                break;
                            case 5:
                                System.out.printf("账号:%s, 用户账号风险级别:嫌疑%n", account);
                                break;
                            case 10:
                                System.out.printf("账号:%s, 用户账号风险级别:致命%n", account);
                                break;
                            default:
                                break;
                        }
                    }
                }
                System.out.printf("%n");
            }
        } else {
            System.out.printf("ERROR: code=%s, msg=%s%n", code, msg);
        }

    }
}
